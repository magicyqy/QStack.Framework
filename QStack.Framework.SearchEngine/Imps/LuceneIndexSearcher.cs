...
namespace QStack.Framework.SearchEngine.Imps
{
    public class LuceneIndexSearcher : ILuceneIndexSearcher
    {
        ...
        public LuceneIndexSearcher(Directory directory, Analyzer analyzer, ICache memoryCache)
        {
        ...
        public List<string> CutKeywords(string keyword)
        {
            ...
            if (_memoryCache.Exists(keyword))
            {
                return _memoryCache.Get<List<string>>(keyword);
            }
            ...
            list.AddRange(Regex.Matches(keyword, "([A-z]+)([0-9.]+)").SelectMany(m => m.Groups.Values.Select(g => g.Value)));//英文+数字
            ...
            _memoryCache.TryAdd(keyword, list, TimeSpan.FromHours(1));
            return list;
        }
        ...
        /// <returns></returns>
        public BooleanQuery GetFuzzyquery(MultiFieldQueryParser parser, string keywords)
        {
