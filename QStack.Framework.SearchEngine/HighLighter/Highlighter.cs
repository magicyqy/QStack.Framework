    ...
    public class Highlighter
    {
            ...
            int maxNumFragments)
        {
            ...
            try
            {
                ...
                int endOffset;
                int lastEndOffset = 0;
                int lastStartOffset = 0;  //用来记录当前所取的字符串起点位置
                string lastMarkedUpTex = string.Empty;
                     ...
                     next = tokenStream.IncrementToken())
                {
                    ...
                    if ((tokenGroup.NumTokens > 0) && (tokenGroup.IsDistinct()))
                    {
                        ...
                        //tokenText = text.Substring(lastStartOffset, endOffset - lastStartOffset);
                        ...
                        if(newText.Length>0&&tokenText?.Equals(markedUpText)==false)
                        {
                            if (startOffset == lastStartOffset && lastEndOffset <= endOffset)
                                newText=newText.Remove(newText.Length - lastMarkedUpTex.Length, lastMarkedUpTex.Length);
                        }
                       
                        newText.Append(markedUpText);
                        lastStartOffset = startOffset;
                        lastEndOffset = Math.Max(endOffset, lastEndOffset);
                        lastMarkedUpTex = markedUpText;
                        tokenGroup.Clear();
